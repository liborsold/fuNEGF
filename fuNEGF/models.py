...
class LinearChain:
    ...
    def __init__(self, N, eps_0, t, a, H_impurity=None, make_H_periodic=False, plot_dispersion=True):
        ...
        if plot_dispersion: self.compare_dispersion_analytic_vs_numerical()
        ...
        self.Sigma_in_fun_k = lambda k: self.Sigma_in_1_fun_k(k) + self.Sigma_in_2_fun_k(k)
    ...
    def G_R_fun(self, k, E, Sigma_0=None):
        ...
        """
        if Sigma_0 is None: Sigma_0 = np.zeros((self.N, self.N))
        return np.linalg.inv( E*np.eye(self.N) - self.H - self.Sigma_fun_k(k) - Sigma_0 ) 
    ...
    def construct_H(self, make_H_periodic=False, print_H_matrix=False):
        ...
        """
        self.H = np.diag([self.eps_0]*self.N) + np.diag([self.t]*(self.N-1), k=1) + np.diag([np.conj(self.t)]*(self.N-1), k=-1) + 0j
        if make_H_periodic is True:
            self.H[0, self.N-1] = self.t
            self.H[self.N-1, 0] = np.conj(self.t)
        if print_H_matrix is True: print_H_matrix(self.H)
    ...
    def add_H_impurity(self, H_impurity, plot_dispersion=False):
        ...
        """
        if H_impurity is not None: self.H += H_impurity
        if plot_dispersion is True: self.compare_dispersion_analytic_vs_numerical()
    ...
    def plot_onsite_energy(self, ax=None):
        ...
        """
        if ax is None: fig, ax = plt.subplots(figsize=(5.5, 1.5))
        ax.step(range(self.N), np.real(np.diag(self.H)), where='mid', color='k', linewidth=1)
    ...
    def compare_dispersion_analytic_vs_numerical(self):
        ...
        axes[0].plot(self.k_states, [self.E_vs_k_fun(k) for k in self.k_states], '-', label='E = eps + 2t cos(ka)',markersize=4)
        # plot the energy dispersion from the Hamiltonian
        axes[0].plot(self.k_states, self.eigvals_ordered, 'o', label='diagonalized H', markerfacecolor='none', markersize=5)
        axes[0].set_ylabel(r'$E$ (eV)', fontsize=12)
        axes[0].set_xlabel(r'$k \cdot a$ (rad)', fontsize=12)
        axes[0].legend()

        # plot as a histogram
        axes[1].hist([self.E_vs_k_fun(k) - self.eigvals_ordered[i] for i, k in enumerate(self.k_states)])
        axes[1].set_ylabel(r'#', fontsize=12)
        axes[1].set_xlabel(r'$E_{\rm analytic} - E_{\rm diagonalized}$ (eV)', fontsize=10)
        plt.tight_layout()
        plt.show()
    ...
    def plot_transmission(self, ax=None):
        ...
        ax.plot(T_states, E_to_plot, 'o-', markerfacecolor='none', markersize=2, color='olive')
    ...
    def plot_occupation(self, D0_phase, D0_phase_momentum, N_sc=70, E_to_plot=0, show=True, ax=None):
        ...
        G_R, G_A, G_n, Sigma_0, Sigma_in_0 = self.Greens_functions_solver(k, E, D0_phase, D0_phase_momentum, N_sc)

        A = 1j*( G_R - G_A)
        f = np.divide( np.diag(G_n), np.diag(A) )

        if ax is None: fig, ax = plt.subplots(figsize=(3.5, 3))
        n_sites_contact = 3
        ax.plot(range(-n_sites_contact, self.N+n_sites_contact), n_sites_contact*[self.f_1] + list(f) + n_sites_contact*[self.f_2], \
